...
void Figure::addShape(Shape *s) {  //Shape adder
    if (currentShapeAmount == maxShapeAmount) {
        ...
        currentShapeAmount++;
        std::cout << groupedShapes[0]->getType() << std::endl;
    } else {
        groupedShapes[currentShapeAmount] = s;
        std::cout << groupedShapes[currentShapeAmount]->getType() << std::endl;
        currentShapeAmount++;
    }
}
...
Line Figure::getBoundingBox() {
    ...
    for (int i = 0; i < currentShapeAmount; i++) {
        if (groupedShapes[i]->getType() == sPolygon) {
            ...
            for(int i = 0; i < currentShape->getPointsAmount(); i++) {
                if(currentShape->points[i].X < topLeft.X) {
                    topLeft.X = currentShape->points[i].X;
                }
                if(currentShape->points[i].Y > topLeft.Y) {
                    topLeft.Y = currentShape->points[i].Y;
                }
                if(currentShape->points[i].X > bottomRight.X) {
                    bottomRight.X = currentShape->points[i].X;
                }
                if(currentShape->points[i].Y < bottomRight.Y) {
                    bottomRight.Y = currentShape->points[i].Y;
                }
            }
